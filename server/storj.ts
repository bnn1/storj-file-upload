...
const BUFFER_SIZE = 80000,
  ...
  getDateTime = (unixTimestamp: number) => {
    const converionValue = 1000,
      ...
      formattedDateTime = `${year}-${month}-${date.substr(
        subString
      )} ${hours.substr(subString)}:${minutes.substr(
        subString
      )}:${seconds.substr(subString)}`;
  ...
  //   await access
  //     .share(permission, sharePrefixListArray, sharePrefixListArray.length)
  //     .then(async (sharedAccess: any) => {
  //       //
  //       console.log("Serilazing share access");
  //       await sharedAccess
  //         .serialize()
  //         .then(async (stringResult: any) => {
  //           //
  //           console.log("Parsing access share...");
  //           await uplink
  //             .parseAccess(stringResult)
  //             .then(async (parsedSharedAccess: any) => {
  //               //
  //               console.log("\nDeriving encryption key");
  //               //
  //               const encryption = await libUplink
  //                 .uplinkDeriveEncryptionKey("test", [4, 5, 6])
  //                 .catch((error: any) => {
  ...
  //         //
  //         fileHandle = await fs.openSync(localFullFileName.dest, "w"),
  //         //
  //         loop = true;
  //       const size = {
  ...
  //       while (loop) {
  //         if (
  //           objectsize - size.totalWritten > checkFileSize &&
  ...
  //             }
  //             if (
  //               size.totalWritten > checkFileSize &&
  //               objectsize > checkFileSize
  //             ) {
  //               console.log(
  //                 "File Dowloaded : ",
  //                 (
      ...
      .then(async (upload: any) => {
          ...
          await upload
            ...
            .then((writeResult: any) => {
              ...
              if (
                size.totalWritten > checkFileSize &&
                size.file > checkFileSize
              ) {
                console.log(
                  "File Uploaded On Storj  : ",
                  (
                    (Number(size.totalWritten) / Number(size.file)) *
                    defaultConversion
                  ).toFixed(fixedValue),
                  " %"
                );
  ...
  .then((access: any) => {
    ...
    access
      ...
      .then(async (project: any) => {
        ...
        //
        await project
          .statBucket(storjConfig.bucketName)
          .then((bucketInfo: any) => {
            console.log(
              "\nBucket Information : \n Bucket Name : ",
              bucketInfo.name,
        ...
        await project
          .createBucket(storjConfig.bucketName)
          .then((bucketInfo: any) => {
            console.log(
              "\nBucket Information : \n Bucket Name : ",
              bucketInfo.name,
        ...
        await project
          .ensureBucket(storjConfig.bucketName)
          .then((bucketInfo: any) => {
            console.log(
              "\nBucket Information : \n Bucket Name : ",
              bucketInfo.name,
        ...
        await project
          .statObject(storjConfig.bucketName, storjConfig.uploadPath)
          .then((objectinfo: any) => {
            console.log(
              "Object : ",
              objectinfo.key,
        ...
        await project
          .listObjects(storjConfig.bucketName, listObjectsOptions)
          .then((objectlist: any) => {
        ...
        await project
          .deleteBucket(storjConfig.bucketName)
          .then((bucketInfo: any) => {
            console.log(
              "\nBucket Deleted : \n Bucket Name : ",
              bucketInfo.name,
          ...
          .catch(async (err: any) => {
            ...
            if (err instanceof uplinkError.BucketNotEmptyError) {
              ...
              await project
                .deleteObject(storjConfig.bucketName, storjConfig.uploadPath)
                .then((objectinfo: any) => {
